...
`{"packed":"no"}` は、キューがメッセージをどのように処理したかを示しています。詳細は[パッキング](#packing)を参照してください。
...
* レスポンスヘッダ:
    ...
    * x-lmq-message-type: メッセージの種類 (`normal`/`package`)
...
```json
{
    "QUEUE NAME 1": {"packed": "FLAG 1"},
    "QUEUE NAME 2": {"packed": "FLAG 2"},
    ...
}
```

* QUEUE NAME N: メッセージを追加したキュー名
* FLAG N: キュー内でのパッキングの状態
    * new: 新たにパッキングが開始された
    * yes: 既存のメッセージとまとめられた
    * no: メッセージはまとめられなかった
