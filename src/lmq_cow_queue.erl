...
allowed_methods(Req, #state{type=msg}=State) ->
    {[<<"GET">>, <<"POST">>], Req, State};
allowed_methods(Req, #state{type=queue}=State) ->
    {[<<"DELETE">>], Req, State}.

resource_exists(Req, #state{method= <<"DELETE">>, name=Name}=State) ->
